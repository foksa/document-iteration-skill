...
1. **Push** to private docs repo
1. **Action** converts Obsidian → Jekyll and pushes to main repo
1. **GitHub Pages** publishes from main repo's `docs/`
...
|`![[Embedded Note]]`|`*See [Note](note.md)*`|
...
* Converts `[[wikilinks]]` → `[text](file.md)`
* Converts `[[link|display]]` → `[display](link.md)`
* Converts `![[embeds]]` → link references
...
1. **Private key** → Added to docs repo as Secret `DEPLOY_KEY`
...
1. Link to other pages using Obsidian's internal link syntax
1. Commit and push
1. Action converts and publishes automatically
